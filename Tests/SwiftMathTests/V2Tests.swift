...
class FV2Tests: XCTestCase {
    ...
    func testAlmostEqual() {
        XCTAssertTrue(FV2(of: 1.231999999).almostEquals(FV2(of: 1.232)), "Almost Equal failed")
        XCTAssertFalse(FV2(of: 1.231).almostEquals(FV2(of: 1.232)), "Almost Equal failed")
    }
    ...
    func testNonUniformScale() {
        let expected = FV2(x: 2, y: 6)
        let candidate = FV2(x: 1, y: 2)
        let actual = candidate * FV2(x: 2, y: 3)
        XCTAssertTrue(actual.almostEquals(expected), "Non-Uniform Scale failed")
    }
    
    func testUniformDivison() {
        let expected = FV2(x: 1, y: 2)
        let candidate = FV2(x: 2, y: 4)
        let actual = candidate / 2
    ...
    func testSubtraction() {
        let expected = FV2(x: 1, y: 2)
        let candidate = FV2(x: 3, y: 5)
        let actual = candidate - FV2(x: 2, y: 3)
        XCTAssertEqual(actual, expected, "Subtraction failed")
    }
    
    func testDotProduct() {
        let candidate = FV2(x: 1, y: 0)
        let testValues = [
            FV2(x: 0, y: 0),
            FV2(x: 1, y: 0),
            FV2(x: -1, y: 0),
