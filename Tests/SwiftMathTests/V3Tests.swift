...
class FV3Tests: XCTestCase {
    func testInit() {
        let expected: [Float] = [1, 2, 3]
        let candidate = FV3(x: 1, y: 2, z: 3)
        let actual = [candidate.x, candidate.y, candidate.z]
        XCTAssertEqual(actual, expected, "Init failed")
    }
    
    func testAlmostEqual() {
        XCTAssertTrue(FV3(of: 1.231999999).almostEquals(FV3(of: 1.232)), "Almost Equal failed")
        XCTAssertFalse(FV3(of: 1.231).almostEquals(FV3(of: 1.232)), "Almost Equal failed")
    }
    ...
    func testLength() {
        ...
        let candidate = FV3(x: 2, y: 3, z: 6)
        let actual = candidate.len()
        XCTAssertEqual(actual, expected, "Length failed")
    }
    
    func testNormalise() {
        let expected = FV3(of: (1 / 3).squareRoot())
        let candidate = FV3(x: 1, y: 1, z: 1)
    ...
    func testNonUniformScale() {
        let expected = FV3(x: 2, y: 6, z: 12)
        let candidate = FV3(x: 1, y: 2, z: 3)
        let actual = candidate * FV3(x: 2, y: 3, z: 4)
        XCTAssertTrue(actual.almostEquals(expected), "Non-Uniform Scale failed")
    }

    func testUniformDivison() {
        let expected = FV3(x: 1, y: 2, z: 3)
        let candidate = FV3(x: 2, y: 4, z: 6)
        let actual = candidate / 2
    ...
    func testNonUniformDivison() {
        let expected = FV3(x: 1, y: 2, z: 3)
        let candidate = FV3(x: 2, y: 6, z: 12)
        let actual = candidate / FV3(x: 2, y: 3, z: 4)
        XCTAssertEqual(actual, expected, "Non-Uniform Division failed")
    }

    func testAddition() {
        let expected = FV3(x: 3, y: 5, z: 7)
        let candidate = FV3(x: 1, y: 2, z: 3)
        let actual = candidate + FV3(x: 2, y: 3, z: 4)
    ...
    func testSubtraction() {
        let expected = FV3(x: 1, y: 2, z: 3)
        let candidate = FV3(x: 3, y: 5, z: 7)
        let actual = candidate - FV3(x: 2, y: 3, z: 4)
        XCTAssertEqual(actual, expected, "Subtraction failed")
    }

    func testDotProduct() {
        let candidate = FV3(x: 1, y: 0, z: 0)
        let testValues = [
            FV3(x: 1, y: 0, z: 0),
            FV3(x: -1, y: 0, z: 0),
            FV3(x: 0, y: 1, z: 0),
